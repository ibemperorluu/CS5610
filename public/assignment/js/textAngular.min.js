...
// key: pre -> taTranslations[key].tooltip, taTranslations[key].buttontext
            ...
            }, activeState: function (a) {/* istanbul ignore next: */
            ...
            iconclass: "fa fa-align-right", tooltiptext: c.justifyRight.tooltip, action: function () {
            ...
            iconclass: "fa fa-align-center", tooltiptext: c.justifyCenter.tooltip, action: function () {
...
// remove lists
                var f = function (a, b) {
                    ...
                    return b || (c = a), angular.forEach(a.children(), function (a) {
                        if ("ul" === a.tagName.toLowerCase() || "ol" === a.tagName.toLowerCase())c = f(a, c); else {
...
// if this link has already been set, we need to just edit the existing link
                /* istanbul ignore if: we do not test this */
                    a = d.getSelectionElement().tagName && "a" === d.getSelectionElement().tagName.toLowerCase() ? b.prompt(c.insertLink.dialogPrompt, d.getSelectionElement().href) : b.prompt(c.insertLink.dialogPrompt, "http://"), a && "" !== a && "http://" !== a && !l(a))return this.$editor().wrapSelection("createLink", a, !0)
            }, activeState: function (a) {
...
// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/Trim#Compatibility
    ...
    if (g.ie > 8 || void 0 === g.ie) {/* istanbul ignore next: preference for stylesheet loaded externally */
        for (var r = document.styleSheets, s = 0; s < r.length; s++)if ((0 === r[s].media.length || r[s].media.mediaText.match(/(all|screen)/gi)) && r[s].href && r[s].href.match(/textangular\.(min\.|)css/gi)) {
            l = r[s];
            break
        }
...
// This order is important as IE 11 has both cssRules and rules but they have different lengths - cssRules is correct, rules gives an error in IE 11
    ...
    angular.module("textAngular.factories", []).factory("taBrowserTag", [function () {
        return function (a) {/* istanbul ignore next: ie specific test */
...
// grab all elements with a style attibute
                var b, c, d, e, f = /<([^>\/]+?)style=("([^\"]+)"|'([^']+)')([^>]*)>/gi, g = /<span class="Apple-converted-space">([^<]+)<\/span>/gi, h = "", i = 0; b = g.exec(a);)d = b[1], d = d.replace(/&nbsp;/gi, " "), h += a.substring(i, b.index) + d, i = b.index + b[0].length;
            for (/* istanbul ignore next: apple-contereted-space span has matched */
                i && (
// modified....
...
// return with remaining html
        ...
        for (var e = [{property: "font-weight", values: ["bold"], tag: "b"}, {
            ...
            for (var h = "(" + e[g].property + ":\\s*(", i = 0; i < e[g].values.length; i++)/* istanbul ignore next: not needed to be tested yet */
...
// if true or undefined is returned then the action has finished. Otherwise the deferred action will be resolved manually.
        ...
        var e = function (a, c) {
            ...
            for (i = 0; i < m.length; i++)if (m[i].outerHTML === d[0].outerHTML) {
// found it...
...
// this is the last in the list, so we just remove it..
            ...
            for (i = 0; i < m.length; i++)for (j = 0; j < d.length; j++)m[i].isEqualNode(d[j]) && (
// found it...
                n[j] = i);
...
// un-list the listElements
            var o = "";
            if (f)for (j = 0; j < d.length; j++)o += "<" + g + ">" + d[j].innerHTML + "</" + g + ">", d[j].remove(); else {
...
// Firefox adds <br>'s and so we remove the <br>
                            B = B.replace(/<br>/i, ""),// nothing
                                y.innerHTML = B, b.setSelectionToElementEnd(y.childNodes[0]), y = b.getSelectionElement())))
                } catch (a) {
                }
//console.log('************** selectedElement:', selectedElement);
                /* istanbul ignore if: */
                if (y) {
                    var D = angular.element(y), E = y && y.tagName && y.tagName.toLowerCase() || /* istanbul ignore next: */
                        "";
...
// here is if there is only text in ta-bind ie <div ta-bind>test content</div>
                                t = angular.element("<" + F + "><li>" + y.innerHTML + "</li></" + F + ">"), D.html(""), D.append(t); else {
...
//console.log('$nodes', $nodes);
                                t = angular.element("<" + F + ">" + u + "</" + F + ">"), I.pop().replaceWith(t), angular.forEach(I, function (a) {
                                    a.remove()
                                })
                            }
...
// $target is wrap element
                            ...
                            } else if (t.parent()[0].tagName.toLowerCase() !== x || t.parent().hasClass("ta-bind"))if (E.match(j))
...
// default wrap behaviour
                                    v = b.getOnlySelectedElements(), 0 === v.length && (
// no nodes at all....
                                        v = [t[0]]), s = 0; s < v.length; s++)if (3 === v[s].nodeType || !v[s].tagName.match(i))for (; 3 === v[s].nodeType || !v[s].tagName || !v[s].tagName.match(i);)v[s] = v[s].parentNode;
                                if (
...
// remove all whole taTextElement if it is here... unless it is the only element!
                                ...
                                })), angular.element(v[0]).hasClass("ta-bind"))t = angular.element(q), t[0].innerHTML = v[0].innerHTML, v[0].innerHTML = t[0].outerHTML; else if ("blockquote" === x) {
                                    for (
// blockquotes wrap other block elements
                                        u = "", s = 0; s < v.length; s++)u += v[s].outerHTML;
...
// refocus on the shown display element, this fixes a bug when using firefox
                        ...
                        if ("createlink" === o.toLowerCase()) {/* istanbul ignore next: firefox specific fix */
                            if ("a" === E)
...
// insert text at selection, then select then just let normal exec-command run
    ...
    }]).service("taSelection", ["$document", "taDOM", "$log", /* istanbul ignore next: all browser specifics and PhantomJS dosen't seem to support half of it */
        function (b, c, d) {
...
//console.info(e);
                    ...
                    var c = b.commonAncestorContainer, d = {
...
// Chrome and Firefox behave differently so so fix this for Firefox here.  No adjustment needed for Chrome.
                updateLeftArrowKey: function (b) {
                    ...
                    if (c && c.collapsed) {
                        ...
                        var e, f, g = c.startContainer, i = d.findIndex(function (a, b) {
...
// If there are no block elements, or there is a mixture we need to create textNodes for the non wrapped text (we don't want them spans messing up the picture).
                            l = [], j = 0; j < s.length; j++) {
                            var u = s[j];
                            "p" === u.nodeName.toLowerCase() && "" === u.innerHTML.trim() || (/****************
...
//console.log('setStartAfter', parent);
                            ...
                            for (o = f.createDocumentFragment(), m = 0; m < r.childNodes.length; m++)p = angular.element("<li>"), c.transferChildNodes(r.childNodes[m], p[0]), c.transferNodeAttributes(r.childNodes[m], p[0]), o.appendChild(p[0]);
...
// recursive function that returns an array of angular.elements that have the passed attribute set on them
            getByAttribute: function (b, c) {
                ...
                for (
// clear out target
                    b.innerHTML = ""; a.childNodes.length > 0;)b.appendChild(a.childNodes[0]);
                return b
            }, splitNodes: function (b, c, d, e, f, g) {
                if (!e && isNaN(g))throw new Error("taDOM.splitNodes requires a splitNode or splitIndex");
                for (var h = document.createDocumentFragment(), i = document.createDocumentFragment(), j = 0; b.length > 0 && (isNaN(g) || g !== j) && b[0] !== e;)h.appendChild(b[0]),// this removes from the nodes array (if proper childNodes object.
                    j++;
...
// console.log('Took', (t1 - t0).toFixed(4), 'milliseconds to generate:');
        ...
        return {
            link: function (a, b, c) {
                b.attr("unselectable", "on"), b.on("mousedown", function (a, b) {/* istanbul ignore else: this is for catching the jqLite testing*/
...
// OR any non-form element with contenteditable="contenteditable" ta-bind="html|text" ng-model
        return {
            ...
            require: ["ngModel", "?ngModelOptions"], link: function (f, u, v, w) {
                function x(a) {
                    var b;
...
// not pure white space so wrap in <p>...</p> or whatever attrs.taDefaultWrap is set to.
                                a += h.trim() ? "<" + v.taDefaultWrap + ">" + h + "</" + v.taDefaultWrap + ">" : h
                            } else if (g.match(i))a += f.outerHTML; else {/* istanbul ignore  next: Doesn't seem to trigger on tests */
...
//console.log(value);
                ...
                v.taPaste && (z = q(v.taPaste)), u.addClass("ta-bind");
                var X;
                f["$undoManager" + (v.id || "")] = C.$undoManager = {
                    _stack: [],
                    _index: 0,
                    _max: 1e3,
                    push: function (a) {
...
// emit the element-select event, pass the element
                ...
                }, _ = f["reApplyOnSelectorHandlers" + (v.id || "")] = function () {/* istanbul ignore else */
                    G || angular.forEach(j, function (a) {
// check we don't apply the handler twice
                        u.find(a).off("click", $).on("click", $)
                    })
                }, aa = function (a, b, c) {
                    I = c || !1, "undefined" != typeof b && null !== b || (b = E),// if not contentEditable then the native undo/redo is fine
                    "undefined" != typeof a && null !== a || (a = Z()), p(a) ? (
// this avoids us from tripping the ng-pristine flag if we click in and out with out typing
                        "" !== C.$viewValue && C.$setViewValue(""), b && "" !== C.$undoManager.current() && C.$undoManager.push("")) : (_(), C.$viewValue !== a && (C.$setViewValue(a), b && C.$undoManager.push(a))), C.$render()
                }, ba = function (a) {
                    u[0].innerHTML = a
                }, ca = f["$undoTaBind" + (v.id || "")] = function () {/* istanbul ignore else: can't really test it due to all changes being ignored as well in readonly */
                    if (!G && E) {
                        var a = C.$undoManager.undo();
                        "undefined" != typeof a && null !== a && (ba(a), aa(a, !1), Y && c.cancel(Y), Y = c(function () {
                            u[0].focus(), h.setSelectionToElementEnd(u[0])
                        }, 1))
                    }
                }, da = f["$redoTaBind" + (v.id || "")] = function () {/* istanbul ignore else: can't really test it due to all changes being ignored as well in readonly */
                    if (!G && E) {
                        var a = C.$undoManager.redo();
                        "undefined" != typeof a && null !== a && (ba(a), aa(a, !1), /* istanbul ignore next */
                        Y && c.cancel(Y), Y = c(function () {
                            u[0].focus(), h.setSelectionToElementEnd(u[0])
                        }, 1))
                    }
                };
//used for updating when inserting wrapped elements
                f["updateTaBind" + (v.id || "")] = function () {
                    G || aa(void 0, void 0, !0)
                };
// catch DOM XSS via taSanitize
// Sanitizing both ways is identical
                var ea = function (a) {
                    return C.$oldViewValue = b(e(a), C.$oldViewValue, J)
                };
//this code is used to update the models when data is entered/deleted
                if (
// trigger the validation calls
                    u.attr("required") && (C.$validators.required = function (a, b) {
                        return !p(a || b)
                    }),
// parsers trigger from the above keyup function or any other time that the viewValue is updated and parses it for storage in the ngModel
                    C.$parsers.push(ea), C.$parsers.unshift(W),
// because textAngular is bi-directional (which is awesome) we need to also sanitize values going in from the server
                    C.$formatters.push(ea), C.$formatters.unshift(W), C.$formatters.unshift(function (a) {
...
// test paste from word/microsoft product
                            if (a.match(/class=["']*Mso(Normal|List)/i) || a.match(/content=["']*Word.Document/i) || a.match(/content=["']*OneNote.File/i)) {
                                ...
                                var g = angular.element("<div>" + e + "</div>"), i = angular.element("<div></div>"), j = {
                                    element: null,
                                    lastIndent: [],
                                    lastLi: null,
                                    isUl: !1
                                ...
                                for (var k = function (a) {
                                    j.isUl = a, j.element = angular.element(a ? "<ul>" : "<ol>"), j.lastIndent = [], j.lastIndent.peek = function () {
                                        var a = this.length;
                                        if (a > 0)return this[a - 1]
                                    }, j.lastLevelMatch = null
                                }, l = 0; l <= g[0].childNodes.length; l++)if (g[0].childNodes[l] && "#text" !== g[0].childNodes[l].nodeName) {
                                    var m = g[0].childNodes[l].tagName.toLowerCase();
                                    if ("p" === m || "h1" === m || "h2" === m || "h3" === m || "h4" === m || "h5" === m || "h6" === m || "table" === m) {
...
// prefers the mso-list syntax
                                                t && t[2] && (s = parseInt(t[2])), t && (!j.lastLevelMatch || t[1] !== j.lastLevelMatch[1]) || !o[3] || "first" === o[3].toLowerCase() || null === j.lastIndent.peek() || j.isUl !== p && j.lastIndent.peek() === s)k(p), i.append(j.element); else if (null != j.lastIndent.peek() && j.lastIndent.peek() < s)j.element = angular.element(p ? "<ul>" : "<ol>"), j.lastLi.append(j.element); else if (null != j.lastIndent.peek() && j.lastIndent.peek() > s) {
                                                for (; null != j.lastIndent.peek() && j.lastIndent.peek() > s;)if ("li" !== j.element.parent()[0].tagName.toLowerCase()) {
                                                    if (!/[uo]l/i.test(j.element.parent()[0].tagName.toLowerCase()))// else it's it should be a sibling
                                                        break;
                                ...
                                }), angular.forEach(i.find("font"), v), a = i.html(), d && (a = i.html() || g.html()),
...
// entire text-angular or ta-bind has been pasted, REMOVE AT ONCE!!
                                    if (a.match(/<[^>]*?(text-angular)[^>]*?>/)) {
                                        ...
                                        for (var x = r.getByAttribute(w, "ta-bind"), y = 0; y < x.length; y++) {
...
// Webkit on Apple tags
                        ...
                        /<
                            ...
                            ?
                        >/
                            i.test(a) && /(<ul(\s.*)?>|<ol(\s.*)?>).*<li(\s.*)?>/i.test(a) === !1 && (
...
//console.log('DONE\n', text);
                                h.insertHtml(a, u[0]), c(function () {
                                C.$setViewValue(Z()), fa = !1, u.removeClass("processing-paste")
                            }, 0)
                        } else fa = !1, u.removeClass("processing-paste")
                    };
                    u.on("paste", f.events.paste = function (b, e) {
...
// Code adapted from http://stackoverflow.com/questions/2176861/javascript-get-clipboard-data-on-paste-event-cross-browser/6804718#6804718
                        fa = !0, u.addClass("processing-paste");
...
// restore selection
                            a.restoreSelection(j), ga(k[0].innerHTML), u[0].focus(), k.remove()
                        }, 0)
                    }), u.on("cut", f.events.cut = function (a) {
// timeout to next is needed as otherwise the paste/cut event has not finished actually changing the display
                        G ? a.preventDefault() : c(function () {
                            C.$setViewValue(Z())
                        }, 0)
                    }), u.on("keydown", f.events.keydown = function (a, b) {/* istanbul ignore else: this is for catching the jqLite testing*/
                        b && angular.extend(a, b), a.keyCode === R ? h.setStateShiftKey(!0) : h.setStateShiftKey(!1), a.specialKey = x(a);
                        var c;
...
// we disable our own
                                    a.specialKey = void 0), b.testForKey(a) && (c = b.commandKeyCode), "UndoKey" !== b.commandKeyCode && "RedoKey" !== b.commandKeyCode || b.enablePropagation || a.preventDefault()
                            }), /* istanbul ignore next: difficult to test */
                            "undefined" != typeof c && (a.specialKey = c), /* istanbul ignore next: difficult to test as can't seem to select */
                            "undefined" == typeof a.specialKey || "UndoKey" === a.specialKey && "RedoKey" === a.specialKey || (a.preventDefault(), s.sendKeyCommand(f, a)), !(G || ("UndoKey" === a.specialKey && (ca(), a.preventDefault()), "RedoKey" === a.specialKey && (da(), a.preventDefault()), a.keyCode !== Q || a.shiftKey || a.ctrlKey || a.metaKey || a.altKey))) {
...
// shifted to nodeName here from tagName since it is more widely supported see: http://stackoverflow.com/questions/4878484/difference-between-tagname-and-nodename
                            ...
                            var i = angular.element(A), j = ["blockquote", "ul", "ol"];
...
// if last element is blank, pull element outside.
                    ...
                    var ha;
                    u.on("keyup", f.events.keyup = function (a, b) {// clear the ShiftKey state
                        /* istanbul ignore next: FF specific bug fix */
                        if (/* istanbul ignore else: this is for catching the jqLite testing*/
                            b && angular.extend(a, b), h.setStateShiftKey(!1), a.keyCode === S) {
                            var d = h.getSelection();
                            return void(d.start.element === u[0] && u.children().length && h.setSelectionToElementStart(u.children()[0]))
                        }
                        if (
...
// if enter - insert new taDefaultWrap, if shift+enter insert <br/>
                                if ("" !== A && "<BR><BR>" !== A && a.keyCode === Q && !a.ctrlKey && !a.metaKey && !a.altKey) {
                                    for (var e = h.getSelectionElement(); !e.nodeName.match(k) && e !== u[0];)e = e.parentNode;
                                    if (a.shiftKey) {
// shift + Enter
...
// For a P: We see: P p <br>
                                        if ((f === v.taDefaultWrap || "li" === f || "pre" === f || "div" === f) && !/.+<br><br>/.test(e.innerHTML.trim())) {
...
// so we replace this with the _defaultVal
                                ...
                                var j = Z();
                                "" === A || "" !== j.trim() && "<br>" !== j.trim() ? "<" !== j.substring(0, 1) && "" !== v.taDefaultWrap : (ba(A), h.setSelectionToElementStart(u.children()[0]));
                                var l = y !== a.keyCode && L.test(a.keyCode);
                                ha && c.cancel(ha), ha = c(function () {
                                    aa(j, l, !0)
                                }, D.$options.debounce || 400), l || (X = c(function () {
                                    C.$undoManager.push(j)
                                }, 250)), y = a.keyCode
                            }
                    });
...
//console.log('new:', _val);
                                        aa(c, !0), a.restoreSelection(b)
                                    }, 1e3))
                        }), u.on("blur", f.events.blur = function () {
                            H = !1, /* istanbul ignore else: if readonly don't update model */
                                G ? (I = !0,// don't redo the whole thing, just check the placeholder logic
                                    C.$render()) : aa(void 0, void 0, !0)
                        }), v.placeholder && (g.ie > 8 || void 0 === g.ie)) {
                        var ja;
                        if (!v.id)throw"textAngular Error: An unique ID is required for placeholders to work";
                        ja = m("#" + v.id + ".placeholder-text:before", 'content: "' + v.placeholder + '"'), f.$on("$destroy", function () {
                            n(ja)
                        })
                    }
                    u.on("focus", f.events.focus = function () {
                        H = !0, u.removeClass("placeholder-text"), _()
                    }), u.on("mouseup", f.events.mouseup = function () {
                        var a = h.getSelection();
                        a && a.start.element === u[0] && u.children().length && h.setSelectionToElementStart(u.children()[0])
                    }),
...
// if a textarea or input just add in change and blur handlers, everything else is done by angulars input directive
                    u.on("change blur", f.events.change = f.events.blur = function () {
                        G || C.$setViewValue(Z())
                    }), u.on("keydown", f.events.keydown = function (a, b) {
...
// update while focussed
                                ...
                                na && c.cancel(na), na = c(function () {/* istanbul ignore if: Can't be bothered testing this... */
                                    H || (u[0].focus(), h.setSelectionToElementEnd(u.children()[u.children().length - 1])), na = void 0
                                }, 1)), E ? (
// blank
                                ba(
// WYSIWYG Mode
...
// only for input and textarea inputs
                                u.val(a)), E && v.placeholder && ("" === a ? H ? u.removeClass("placeholder-text") : u.addClass("placeholder-text") : u.removeClass("placeholder-text")), pa = I = !1
                    }
                }, v.taReadonly && (
//set initial value
...
// Otherwise it is readonly by default
                    f.$watch(v.taReadonly, function (a, b) {
                        b !== a && (a ? (u.addClass("ta-readonly"),
// we changed to readOnly mode (taReadonly='true')
...
// turn ON selector click handlers
                            angular.forEach(j, function (a) {
                                u.find(a).on("click", $)
                            }), u.off("drop", oa)) : (u.removeClass("ta-readonly"),
// we changed to NOT readOnly mode (taReadonly='false')
                            "textarea" === u[0].tagName.toLowerCase() || "input" === u[0].tagName.toLowerCase() ? u.removeAttr("disabled") : E && u.attr("contenteditable", "true"),
...
// if in WYSIWYG and readOnly we kill the use of links by clicking
                E && !G && (angular.forEach(j, function (a) {
                    u.find(a).on("click", $)
                }), u.on("drop", oa))
            }
        }
...
// clear taTools variable. Just catches testing and any other time that this config may run multiple times...
        ...
        return {
            require: "?ngModel", scope: {}, restrict: "EA", priority: 2,// So we override validators correctly
...
// credit to lotif on http://stackoverflow.com/questions/4880381/check-whether-html-element-has-scrollbars
                var E = function (a) {
                    ...
                    } catch (a) {/* istanbul ignore next: error handler */
                        return c
                    }
                    var d = b["overflow-y"], e = b["overflow-x"];
                    return {
                        vertical: ("scroll" === d || "auto" === d) && /* istanbul ignore next: not tested */
                        a.scrollHeight > a.clientHeight,
                        horizontal: ("scroll" === e || "auto" === e) && /* istanbul ignore next: not tested */
...
// remove element eventListener
                    ...
                    return "undefined" == typeof c && (c = 0), b && E(a).vertical && (a.removeEventListener("scroll", m._scrollListener, !1), a.addEventListener("scroll", m._scrollListener, !1)), 0 !== c ? {
                        node: a.nodeName,
                        top: c
                    } : a.parentNode ? m.getScrollTop(a.parentNode, b) : {node: "<none>", top: 0}
                },
...
// allow for insertion of custom directives on the textarea and div
                    ...
                    "ng-model": "html",
                    "ng-model-options": n.attr("ng-model-options")
                }), m.displayElements.scrollWindow.attr({"ng-hide": "showHtml"}), o.taDefaultWrap &&
...
// add the classes manually last
                ...
                var F = function (a, b) {/* istanbul ignore next: this is only here because of a bug in rangy where rangy.saveSelection() has cleared the state */
...
// this fixes bugs #423, #1129, #1105, #693 which are actually rangy bugs!
                        return m._actionRunning = !0, b = h[0].queryCommandState("bold"), c = h[0].queryCommandState("italic"), d = h[0].queryCommandState("underline"), e = h[0].queryCommandState("strikeThrough"), G = a.saveSelection(), F("bold", b), F("italic", c), F("underline", d), F("strikeThrough", e), function () {
...
// cascades to displayElements.text and displayElements.html automatically.
                        u = function (a) {
                            m.focussed = !0, n.addClass(m.classes.focussed), /*******  NOT FUNCTIONAL YET
...
// WOW, normally I would do a scope.$apply here, but this causes ERRORs when doing tests!
                        ...
                        }, m.displayElements.html.on("focus", u), m.displayElements.text.on("focus", u), v = function (a) {/****************** NOT FUNCTIONAL YET
                     try {
...
// changes from taBind back up to here
                    ...
                    }), o.taTargetToolbars)x = g.registerEditor(m._name, m, o.taTargetToolbars.split(",")); else {
...
// so move the selection before the <a>!!
                        ...
                        b && angular.extend(a, b), m.$apply(function () {
                            if (x.sendKeyCommand(a))/* istanbul ignore else: don't run if already running */
...
// we return true if we applied an action, false otherwise
                            ...
                            return (e.ctrlKey || e.metaKey || e.specialKey) && angular.forEach(b, function (b, g) {
                                if (b.commandKeyCode && (b.commandKeyCode === e.which || b.commandKeyCode === e.specialKey))for (var h = 0; h < j[c].toolbarScopes.length; h++)if (void 0 !== j[c].toolbarScopes[h].tools[g]) {
...
// Run the actions on the first visible filtered tool only
                            if (angular.forEach(b, function (a, b) {
                                    a.onElementSelect && a.onElementSelect.element && a.onElementSelect.element.toLowerCase() === e[0].tagName.toLowerCase() && (!a.onElementSelect.filter || a.onElementSelect.filter(e)) && (
...
// sort most specific (most attrs to find) first
                                    ...
                                    }), g.length > 0)for (var l = 0; l < g.length; l++) {
                                for (var m = g[l].tool, n = g[l].name, o = 0; o < j[c].toolbarScopes.length; o++)if (void 0 !== j[c].toolbarScopes[o].tools[n]) {
...
// this will call a $digest if not already happening
            refreshEditor: function (a) {
                ...
                j[a].scope.updateTaBindtaTextElement(), /* istanbul ignore else: phase catch */
...
// we assume here that all values passed are valid and correct
        ...
        var b = a.getVersion();
        return {
            restrict: "EA", link: function (a, c, d) {
                c.html(b)
            }
        }
    }]), u.name
